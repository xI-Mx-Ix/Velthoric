...
public class VxBodySelectorParser {
    ...
    public static final DynamicCommandExceptionType ERROR_UNKNOWN_BODY_TYPE = new DynamicCommandExceptionType((obj) -> Component.literal("Unknown body type '" + obj + "'"));
    ...
    private void parseOptionValue(String name) throws CommandSyntaxException {
        ...
        switch (name) {
            ...
            case "type" -> {
                ...
                if (VxBodyRegistry.getInstance().getRegistrationData(parsedLocation) == null) {
                    reader.setCursor(valueStartCursor);
                    throw ERROR_UNKNOWN_BODY_TYPE.createWithContext(reader, parsedLocation.toString());
                }
    ...
    private CompletableFuture<Suggestions> suggestOptionValues(String option, SuggestionsBuilder builder) {
        switch (option) {
            case "type" -> {
                var keys = VxBodyRegistry.getInstance().getRegisteredTypes().keySet().stream().map(ResourceLocation::toString);
