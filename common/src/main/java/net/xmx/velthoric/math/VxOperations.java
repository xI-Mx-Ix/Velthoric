...
/**
 * Utility class for advanced vector and quaternion mathematics.
 * <p>
 * Contains methods for interpolation (Lerp, Slerp, Cubic Hermite),
 * extrapolation, and coordinate conversion used within the physics engine.
 *
 * @author xI-Mx-Ix
 */
public class VxOperations {
     ...
     * @param store The vector to store the result in (if null, a new one is created).
     ...
     * <p>
     * This method handles the "shortest path" problem by checking the dot product.
     * If the quaternions are very close, it falls back to linear interpolation to avoid division by zero.
     *
     * @param q1    The start quaternion.
     * @param q2    The end quaternion.
     * @param alpha The interpolation factor (0.0 to 1.0).
     * @param store The quaternion to store the result in (if null, a new one is created).
    ...
    public static Quat slerp(QuatArg q1, QuatArg q2, float alpha, Quat store) {
        ...
        // If they are very close, use linear interpolation (Lerp) to avoid numerical instability
        if (dot > 0.9995f) {
    ...
    /**
     * Extrapolates a rotation based on angular velocity over a time step.
     *
     * @param startRot The initial rotation.
     * @param angVel   The angular velocity vector (axis * speed).
    ...
    /**
     * Extrapolates a position based on linear velocity over a time step.
     *
     * @param p0    The initial position.
     * @param v0    The linear velocity.
     * @param dt    The time delta.
     * @param store The vector to store the result in (if null, a new one is created).
    ...
    /**
     * Performs a cubic interpolation between two rotations, taking angular velocity into account.
     * <p>
     * This provides a smoother transition than standard Slerp when velocities are involved.
     *
     * @param q0      Start rotation.
     * @param angVel0 Start angular velocity.
     * @param q1      Target rotation.
     * @param angVel1 Target angular velocity.
     * @param dt      Total time duration between q0 and q1.
     * @param t       Normalized time factor (0.0 to 1.0).
    ...
    public static Quat interpolateCubic(QuatArg q0, Vec3Arg angVel0, QuatArg q1, Vec3Arg angVel1, float dt, float t, Quat store) {
        ...
        // Control points for cubic Bezier interpolation on the sphere
        Quat c0 = new Quat();
        ...
        // De Casteljau's algorithm for Slerp
        slerp(q0, c0, t, tempA);
        slerp(c0, c1, t, tempB);
        slerp(c1, q1, t, store);
    ...
    /**
     * Performs a Cubic Hermite spline interpolation for position.
     * <p>
     * This ensures that the position curve respects the velocity at both the start and end points.
     *
     ...
     * @return A Vec3 containing the angles in radians (X=Roll, Y=Pitch, Z=Yaw).
     */
    public static Vec3 quatToEulerAngles(QuatArg q) {
        ...
        if (Math.abs(sinp) >= 1) {
            angles.setY((float) Math.copySign(Math.PI / 2, sinp)); // Use 90 degrees if out of range
        } else {
