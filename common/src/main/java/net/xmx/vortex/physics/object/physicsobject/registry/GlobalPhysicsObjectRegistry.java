package net.xmx.vortex.physics.object.physicsobject.registry;
...
public final class GlobalPhysicsObjectRegistry {
    ...
    public interface PhysicsObjectFactory {
         ...
         */
        IPhysicsObject create(UUID id, Level level, String typeId, VxTransform transform, IPhysicsObjectProperties properties, @Nullable FriendlyByteBuf initialData);
    }
    ...
    public static void register(String typeId, EObjectType objectType, IPhysicsObjectProperties properties, Class<? extends IPhysicsObject> clazz) {
        if (REGISTERED_TYPES.containsKey(typeId)) {
            VxMainClass.LOGGER.warn("Attempted to re-register PhysicsObject type '{}'. Skipping.", typeId);
            return;
        }

        try {
            // The constructor signature is now updated to use FriendlyByteBuf.
            Constructor<? extends IPhysicsObject> ctor = clazz.getDeclaredConstructor(UUID.class, Level.class, String.class, VxTransform.class, IPhysicsObjectProperties.class, FriendlyByteBuf.class);
            ...
            PhysicsObjectFactory factory = (id, level, type, transform, props, initialData) -> {
                try {
                    ...
                    VxMainClass.LOGGER.error("Failed to instantiate physics object of type {}", type, e);
                    return null;
                }
            };
            REGISTERED_TYPES.put(typeId, new RegistrationData(objectType, properties, clazz, factory));
            VxMainClass.LOGGER.debug("Globally registered PhysicsObject type: {} -> {}", typeId, clazz.getName());
