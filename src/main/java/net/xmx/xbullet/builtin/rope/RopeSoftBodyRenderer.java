...
public class RopeSoftBodyRenderer extends SoftPhysicsObject.Renderer {
    ...
    private static final ResourceLocation YELLOW_WOOL_BLOCK_TEXTURE = ResourceLocation.parse("minecraft:block/yellow_wool");
    ...
    private void renderSmoothRope(float[] vertexData, int numNodes, float ropeRadius, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        ...
        for (int nodeIdx = 0; nodeIdx < numNodes - 1; nodeIdx++) {
            ...
            for (int sideIdx = 0; sideIdx < SIDES; sideIdx++) {
                ...
                addQuad(buffer, poseStack, v00, v10, v11, v01, normal,
                        finalTexU00, finalTexV00, finalTexU10, finalTexV10,
                        finalTexU11, finalTexV11, finalTexU01, finalTexV01,
                        packedLight);
            }
        }
    }
    ...
    private void addQuad(VertexConsumer buffer, PoseStack poseStack, Vec3 v1, Vec3 v2, Vec3 v3, Vec3 v4, Vec3 normal,
        ...
        buffer.vertex(last.pose(), v4.getX(), v4.getY(), v4.getZ())
                .color(255, 255, 255, 255).uv(u4, v4Coord).uv2(packedLight)
                .normal(last.normal(), normal.getX(), normal.getY(), normal.getZ()).endVertex();
    }
}