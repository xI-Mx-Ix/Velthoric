package net.xmx.xbullet.physics.collision.entity;
...
public class PhysicsVoxelShape extends ArrayVoxelShape {
    ...
    public PhysicsVoxelShape(Level level, int targetBodyId) {
        ...
        this.physicsWorld = PhysicsWorld.get(level.dimension());
    }
    ...
    private AABB calculateCurrentBounds() {
        ...
        TerrainSystem terrainSystem = PhysicsWorld.getTerrainSystem(physicsWorld.getDimensionKey());
        if (terrainSystem != null && terrainSystem.isTerrainBody(this.targetBodyId)) {
            XBullet.LOGGER.warn("[DEBUG] PhysicsVoxelShape for a terrain body ({}) was created! Returning empty bounds.", this.targetBodyId);
            return EMPTY_AABB;
        }
        ...
        try (BodyLockRead lock = new BodyLockRead(physicsWorld.getBodyLockInterface(), targetBodyId)) {
            ...
            XBullet.LOGGER.error("JoltVoxelShape: Error getting bounds for body {}: {}", targetBodyId, e.getMessage(), e);
        }
        return EMPTY_AABB;
    }
    ...
    public AABB bounds() {
        ...
        if (bounds.getSize() > 0) {
            XBullet.LOGGER.info("[DEBUG 1B] PhysicsVoxelShape.bounds() called for body {}. Bounds: {}", this.targetBodyId, bounds);
        }
        return bounds;
    }
    ...
    public double collide(Direction.Axis movementAxis, AABB entityAABB, double desiredOffset) {
        ...
        try {
            ...
            XBullet.LOGGER.error("JoltVoxelShape: Sweep-Test error for body {}: {}", targetBodyId, e.getMessage(), e);
            return desiredOffset;
        }
    }
