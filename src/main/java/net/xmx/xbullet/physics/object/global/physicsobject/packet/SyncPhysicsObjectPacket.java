...
public class SyncPhysicsObjectPacket {
    ...
    @Nullable private final PhysicsTransform transform;
    @Nullable private final Vec3 linearVelocity;
    @Nullable private final Vec3 angularVelocity;
    ...
    public SyncPhysicsObjectPacket(IPhysicsObject obj, long timestamp, boolean isActive) {
        ...
        if (obj instanceof RigidPhysicsObject rpo) {
            this.transform = rpo.getCurrentTransform();
            this.linearVelocity = rpo.getLastSyncedLinearVel();
            this.angularVelocity = rpo.getLastSyncedAngularVel();
            ...
            throw new IllegalArgumentException("Unknown IPhysicsObject type for sync packet");
        }
    }
    ...
    public static void handle(SyncPhysicsObjectPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ClientPhysicsObjectManager.getInstance().updateObject(
